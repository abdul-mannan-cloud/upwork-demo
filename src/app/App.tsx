...
function App() {
    ...
    // Create SDK audio element on client side only
    useEffect(() => {
    ...
    // Initialize audio playback state from localStorage on client side only
    useEffect(() => {
        if (typeof window !== 'undefined') {
            const stored = localStorage.getItem('audioPlaybackEnabled');
            setIsAudioPlaybackEnabled(stored ? stored === 'true' : true);
        }
    }, []);
    ...
    // Initialize agent config from URL params
    useEffect(() => {
    ...
    // Update session when connected and agent changes
    useEffect(() => {
        if (
    ...
    const fetchEphemeralKey = async (): Promise<string | null> => {
        ...
        try {
            ...
            const data = isJson ? await tokenResponse.json() : await tokenResponse.text();
            logServerEvent(typeof data === 'object' ? data : { body: data }, "fetch_session_token_response");

            if (!tokenResponse.ok) {
                console.error("/api/session error", tokenResponse.status, data);
                setSessionStatus("DISCONNECTED");
                return null;
            }

            const key = (data as any)?.client_secret?.value;
            if (!key) {
                logClientEvent(typeof data === 'object' ? data : { body: data }, "error.no_ephemeral_key");
    ...
    // Handle recording when session is connected
    useEffect(() => {
    ...
    return (
        <div className="text-base flex flex-col h-screen bg-gray-100 text-gray-800 relative">
            <div className="sticky top-0 z-20 bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/60 border-b border-gray-200 shadow-sm">
                <div className="max-w-7xl mx-auto px-4">
                    ...
                    <div className="flex items-center justify-between gap-4 flex-wrap py-2">
                        ...
                        {agentSetKey && selectedAgentConfigSet && (
